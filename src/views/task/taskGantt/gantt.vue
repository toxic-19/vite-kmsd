...
export default {
  ...
  data() {
    return {
      mouseType: '', // 进入gantt-timeline时变为timeline。进入gantt-item是变为item
      mouseWidth: 0,
      mouseScaleWidth: 0,
  ...
  methods: {
    ...
    itemMouseMove(e) {
      if (this.mouseItem != null) {
        e.preventDefault()
        let diff = e.clientX - this.mouseBak.clientX
        this.mouseBak[this.mouseBak.type] = this.mouseBak.value + diff
    ...
    itemMouseUp(e) {
      ...
      if (this.mouseItem != null) {
        ...
        if (typeof this.mouseItem.moveX === 'number' && this.mouseItem.moveX !== 0) {
          ...
          this.mouseItem.time['start'] = start + moveTime
          this.mouseItem.time['end'] = end + moveTime
          this.mouseItem['moveX'] = 0
          isM = true
        }
        //修改结束时间
